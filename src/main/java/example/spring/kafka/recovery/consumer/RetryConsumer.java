...
public class RetryConsumer {
    ...
    private void processRecord(String data, String topic, int partition, int offset) {
        if (data.contains(RecordType.RETRY_SUCCESS.name())) {
            log.info("Data successfully consumed in retry, topic = {}, partition = {}, offset = {}, data = {}", topic,
                    partition, offset, data);
        } else if (data.contains(RecordType.RETRY_FAIL.name())) {
            log.info("Retry fail data consumed in retry , topic = {}, partition = {}, offset = {}, success data = {}",
                    topic, partition, offset, data);
            throw new RetriableException(RecordType.RETRY_FAIL, partition, offset);
        } else {
            log.info("Unknown message retrieve , Message should be in {} ", Arrays.asList(FailRecordType.values()));
        }
    }
